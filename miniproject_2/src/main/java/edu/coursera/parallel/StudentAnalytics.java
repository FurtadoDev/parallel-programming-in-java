...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
        ...
        double retValue = Stream.of(studentArray).parallel().filter(x -> x.checkIsCurrent() == true ).mapToDouble(a -> a.getAge()).average().getAsDouble();
        return retValue;
    }
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        //throw new UnsupportedOperationException();
        Map<String, Long> firstname_count_map = Stream.of(studentArray).parallel().collect(Collectors.groupingBy( s -> s.getFirstName(), Collectors.counting()));
        String answer = Collections.max(firstname_count_map.entrySet(), Map.Entry.comparingByValue()).getKey();;
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        //throw new UnsupportedOperationException();
        long return_val = Stream.of(studentArray).parallel().filter(x -> x.checkIsCurrent() == false && x.getAge() > 20 && x.getGrade() < 65).count();
